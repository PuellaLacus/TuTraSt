    ...
    for cluster2=cluster1:last_cluster
        ...
        for N=1:length(pair_index)
            ...
            if TS_list_all(TS_index,11)==0 %Find first unchecked TS in cluster1 cluster2 interface
                                ...
                                if l==1 && m==4 && n==7
                                    ...
                                    if sum(neighbor_match) %check if neighbor is a transition state
                                        ...
                                        if length(neighbor_index)>1
                                            if TS_list_all(neighbor_index,11)==0
                ...
                cluster1_min=min(list.finC(cluster1).min(1));
                coord_cluster1_min=list.finC(cluster1).min(2:4);
                cluster2_min=min(list.finC(cluster2).min(1));
                coord_cluster2_min=list.finC(cluster2).min(2:4);
                dE_1=TS_min-cluster1_min;
                dE_2=TS_min-cluster2_min;
                list.TSgroup(group_index).data=TS_list_group;
                list.TSgroup(group_index).info.group(1:6)=[group_index coord_TSgroup_min(1) coord_TSgroup_min(2) coord_TSgroup_min(3) TS_min 0];
                ...
                for iT=1:length(list.tunnels)
                    if ismember(cluster1,list.tunnels(iT).clusters)
                        list.TSgroup(group_index).info.group(6)=iT;
                        list.tunnels(iT).TSgroup=[list.tunnels(iT).TSgroup group_index];
                    end
                end
            end
        end
